...
def validate(
    ...
    with torch.no_grad():
        for _batch in tqdm(test_loader, desc="Validation", dynamic_ncols=True, position=2):
            ...
            output, masks = model(batch["tokens_ids"][:, :-1], batch["lengths"] - 1)
            loss = model.loss(output, batch["tokens_ids"][:, 1:], masks)
            all_loss += loss.item()

            if epoch is not None:
                for i in range(min(batch["tokens_ids"].size(0), 5)):
                    tokens_ids = batch["tokens_ids"][i]
                    output_tokens_ids = output[i].infer(starts=tokens_ids[:5], max_token_count=200)
                    gt_text = tokenizer.decode(tokens_ids)
                    output_text = tokenizer.decode(tokens_ids[:5] + output_tokens_ids)
...
def main():
    ...
    model = Decoder(tokenizer.n_vocab, config.model.n_layers, config.model.d_model, config.model.n_heads)
    model.to(device)
    ...
    if config.dataset == "SimpleStories-Both":
        train_dataset_ja = datasets.load_dataset("SimpleStories/SimpleStories-JA", split="train")
        train_dataset_en = datasets.load_dataset("SimpleStories/SimpleStories", split="train")
        train_dataset = train_dataset_ja.map(lambda x: tokenizer.encode(x["story"]), batched=True) + train_dataset_en.map(
            lambda x: tokenizer.encode(x["story"]), batched=True
        )

        test_dataset_ja = datasets.load_dataset("SimpleStories/SimpleStories-JA", split="test")
        test_dataset_en = datasets.load_dataset("SimpleStories/SimpleStories", split="test")
        test_dataset = test_dataset_ja.map(lambda x: tokenizer.encode(x["story"]), batched=True) + test_dataset_en.map(
            lambda x: tokenizer.encode(x["story"]), batched=True
        )
    elif "SimpleStories" in config.dataset:
        train_dataset = datasets.load_dataset(config.dataset, split="train")
        train_dataset = train_dataset.map(lambda x: tokenizer.encode(x["story"]), batched=True)
        test_dataset = datasets.load_dataset(config.dataset, split="test")
        test_dataset = test_dataset.map(lambda x: tokenizer.encode(x["story"]), batched=True)
    else:
